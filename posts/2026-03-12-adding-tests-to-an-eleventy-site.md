...
What we want is to run the tests _after_ Netlify has executed the build command but _before_ it has published the `_site` directory. This way, if tests were to fail, so would the deployment and the broken version wouldn’t be published.
