...
- **Discover new things.** I’ve learned to use the PHP/MySQL combo but I finally learned how not to use it when it is not needed. I wanted to discover a new thing and it turned out to be quite simple to use in the end.
...
## Redirect, DNS and all this stuff
...
I opted for a very simple, albeit imperfect, approach which consisted of creating HTML files at the old locations redirecting to the new files with meta tags. For example, there is a file in a `/blog` folder called `css-gradients.html` containing only a the basic html/head/body tag and:
