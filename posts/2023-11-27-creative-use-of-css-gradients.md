...
{% callout %}[Roman Komarov](https://twitter.com/kizmarh) actually found a few additional ways to compress the gradient value even further (see [his CodePen for the code](https://codepen.io/kizu/pen/zYeJvxr?editors=1100)). I quote:
...
3. A tiny one: we can store only the color stops inside the gradient, and use the variable inside the `linear-gradient()` itself, making it so we won’t repeat that part every time.{% endcallout %}
