...
{% callout %}
Note that `style` has nothing to do with the `style` HTML attribute. It’s a property called `style` on Portable Text nodes which may contain things like `normal`, `h2`, `h3`, etc.
{% endcallout %}
