...
<pre class="language-javascript"><code>var $items = $('.parent').children();
...
<pre class="language-scss"><code>@for $i from 1 through $nb-items {
...
<pre class="language-scss"><code>@mixin distribute-on-circle($nb-items, $circle-size, $item-size, $class-for-IE: false) {
  /* … */
  @for $i from 1 through $nb-items {
    @if not $class-for-IE {
      &:nth-of-type(#{$i}) {
