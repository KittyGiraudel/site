...
Even if it’s not much of a concern, using an HTML/CSS “alternative” to the `<select>` element means using at least a dozen DOM nodes (quickly ramping up with the number of options) and maybe about 50 lines of CSS, perhaps including some heavy CSS properties like shadows or gradients.
