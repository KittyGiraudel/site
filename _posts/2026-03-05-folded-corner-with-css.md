...
{% assign footnote_pinch_values = "These values are totally arbitrary, and they have been defined by trial and error until it looks okay." %}

There really are only 2 values that change: {% footnoteref "pinch-values" footnote_pinch_values %}the pinch on the top edge (at 30% of the width), and the pinch on the left edge (at 25% of the height){% endfootnoteref %}. We can use CSS custom properties to avoid authoring the whole clip path twice and to add some meaning behind the values:
...
{% assign footnote_fold_color = "It doesn’t actually <em>have</em> to be white. It can be any color you think the back of the image should be. I found out that white gives the most “realistic” effect, but that’s subjective if anything." %}

Now, we need to superimpose a {% footnoteref "fold-color" footnote_fold_color %}white{% endfootnoteref %} triangle that represents the folded part of the image. We already have 2 of our 3 points: the top and the left pinches. We need a 3rd point that represent the corner of the image, and the lower point of our triangle. Again, we can use CSS properties to make things cleaner.
...
{% assign footnote_blur_filter = "If you manage to make <code>filter: blur(..)</code> work, please do let me know as I’d like to learn." %}

The problem with a shadow like this is that it’s too sharp. It should be a bit more blurry, really. Unfortunately, we cannot use `filter: blur(..)` with `clip-path` as {% footnoteref "blur-filter" footnote_blur_filter %}it doesn’t work the way we’d expect{% endfootnoteref %}.
