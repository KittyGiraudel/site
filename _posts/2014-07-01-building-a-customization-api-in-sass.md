...
**Our Sass-powered customization API** will essentially be able to _mark_ elements within our stylesheet that we want to customize, and which of those elements _properties_ may be customized as well as default _values_ for these properties.
...
{% info %}If you’re unfamiliar with using maps as arguments, [Kitty wrote up a pretty nifty article on that](https://www.sitepoint.com/using-sass-maps/), as well as many other use-cases for maps.{% endinfo %}
